...
var ServerPage, DbPage: TInputQueryWizardPage;
    ServerAddress, DatabaseServer, DatabaseName: String;

function ExecWait(const FileName, Params: String): Boolean;
var R: Integer;
begin Result := Exec(FileName, Params, '', SW_HIDE, ewWaitUntilTerminated, R) and (R = 0); end;

function ServiceExists(const Name: String): Boolean;
var R: Integer;
begin Result := Exec(ExpandConstant('{sys}\sc.exe'), 'query "' + Name + '"', '', SW_HIDE, ewWaitUntilTerminated, R) and (R = 0); end;

function GetAppUrl(P: String): String;
begin Result := 'http://' + ServerAddress + ':5080'; end;

function NormalizeAddress(V: String): String;
begin
  Result := Trim(V);
  if Pos('http://', Lowercase(Result)) = 1 then Delete(Result,1,7)
  else if Pos('https://', Lowercase(Result)) = 1 then Delete(Result,1,8);
...
var I: Integer;
begin Result:=''; Randomize; for I:=1 to 64 do Result:=Result+C[Random(Length(C))+1]; end;
...
begin
  ...
  ServerPage.Add('Server IP / hostname:',False); ServerPage.Values[0]:='192.168.1.50';
  ...
  DbPage.Add('SQL Server instance:',False); DbPage.Add('Database name:',False);
  DbPage.Values[0]:='.\SQLEXPRESS'; DbPage.Values[1]:='AVASurfaceBilling';
end;

function NextButtonClick(PageID: Integer): Boolean;
begin
  Result:=True;
  if PageID=ServerPage.ID then begin ServerAddress:=NormalizeAddress(ServerPage.Values[0]); if ServerAddress='' then begin MsgBox('Enter the production server IP/hostname.',mbError,MB_OK); Result:=False; end; end
  else if PageID=DbPage.ID then begin DatabaseServer:=Trim(DbPage.Values[0]); DatabaseName:=Trim(DbPage.Values[1]); if DatabaseServer='' then begin MsgBox('Enter the SQL Server instance.',mbError,MB_OK); Result:=False; end else if DatabaseName='' then begin MsgBox('Enter the database name.',mbError,MB_OK); Result:=False; end; end;
end;

procedure WriteConfig;
var P,S,J: String;
begin
  P:=ExpandConstant('{app}\AVA-Surface-Production.json'); if FileExists(P) then Exit;
  S:=NewSecret;
  J:='{'+#13#10+'  "Server":{"IpAddress":"'+ServerAddress+'","Port":5080},'+#13#10+
...
var R: Integer;
begin Exec(ExpandConstant('{sys}\icacls.exe'),'"'+ExpandConstant('{app}\AVA-Surface-Production.json')+'" /inheritance:r /grant:r "SYSTEM:F" "Administrators:F" "LOCAL SERVICE:R"','',SW_HIDE,ewWaitUntilTerminated,R); end;

procedure StopService;
begin if ServiceExists('{#ServiceName}') then begin ExecWait(ExpandConstant('{sys}\sc.exe'),'stop "{#ServiceName}"'); Sleep(1500); end; end;

procedure ConfigureService;
var E,B: String;
begin
  E:=ExpandConstant('{app}\{#AppExe}'); B:='"'+E+'" --urls "http://'+ServerAddress+':5080"';
  if not ServiceExists('{#ServiceName}') then ExecWait(ExpandConstant('{sys}\sc.exe'),'create "{#ServiceName}" binPath= "'+B+'" start= auto obj= "NT AUTHORITY\LOCAL SERVICE" displayname= "{#AppName}"');
...
begin
  ...
  if not Exec(ExpandConstant('{sys}\WindowsPowerShell\v1.0\powershell.exe'),P,'',SW_HIDE,ewWaitUntilTerminated,R) or (R<>0) then RaiseException('SQL initialization failed.');
end;
...
begin
  ...
  if not Exec(ExpandConstant('{app}\database\efbundle.exe'),P,ExpandConstant('{app}\database'),SW_SHOWNORMAL,ewWaitUntilTerminated,R) or (R<>0) then RaiseException('Database migration failed.');
end;
...
begin
 if Step=ssPostInstall then begin WriteConfig; ProtectConfig; InitDb; MigrateDb; ConfigureService; Firewall; if not ExecWait(ExpandConstant('{sys}\sc.exe'),'start "{#ServiceName}"') then RaiseException('Vero Billing System service could not be started.'); end;
end;

procedure CurUninstallStepChanged(Step: TUninstallStep);
begin
 if Step=usUninstall then begin ExecWait(ExpandConstant('{sys}\sc.exe'),'stop "{#ServiceName}"'); ExecWait(ExpandConstant('{sys}\sc.exe'),'delete "{#ServiceName}"'); end;
end;

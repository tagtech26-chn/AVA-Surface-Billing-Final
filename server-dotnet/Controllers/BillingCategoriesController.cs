...
public sealed class BillingCategoriesController(BillingDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Create(CreateBillingCategoryRequest request, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(request.Code) || string.IsNullOrWhiteSpace(request.Name))
            return BadRequest("Billing category code and name are required.");
        var code = request.Code.Trim().ToUpperInvariant();
        if (await db.CustomerCategories.AnyAsync(x => x.Code == code, ct))
            return Conflict("A billing category with this code already exists.");
    ...
    {
        if (!await db.Customers.AnyAsync(x => x.Id == customerId, ct))
        ...
        if (assignment is null)
            db.CustomerCategoryAssignments.Add(new CustomerCategoryAssignment { CustomerId = customerId, CategoryId = request.CategoryId });
        else
    ...
    {
        var row = await db.CustomerCategoryAssignments.AsNoTracking()
            .Where(x => x.CustomerId == customerId)
            .Join(db.CustomerCategories, a => a.CategoryId, c => c.Id,
                (a, c) => new { c.Id, c.Code, c.Name })
    ...
    public async Task<IActionResult> Prices([FromQuery] Guid categoryId, [FromQuery] Guid? productId, CancellationToken ct)
    {
        ...
        if (productId.HasValue)
            q = q.Where(x => x.ProductId == productId || x.ProductId == null);
    ...
    public async Task<IActionResult> EffectivePrice([FromQuery] Guid productId, [FromQuery] Guid customerId, CancellationToken ct)
    {
        ...
        var assignment = await db.CustomerCategoryAssignments.AsNoTracking()
        ...
        var rule = await db.CategoryPriceRules.AsNoTracking()
            .Where(x => x.CategoryId == assignment.CategoryId &&
                        (x.ProductId == productId || x.ProductId == null) &&
                        x.IsActive && x.ValidFrom <= today && x.ValidTo >= today)
            .OrderByDescending(x => x.ProductId.HasValue)
            .ThenByDescending(x => x.Priority)
            .FirstOrDefaultAsync(ct);
        var category = await db.CustomerCategories.AsNoTracking()
            .FirstAsync(x => x.Id == assignment.CategoryId, ct);

        var finalPrice = product.SellingPrice;
        var source = "STANDARD";
        if (rule is not null)
        {
            finalPrice = rule.FixedPrice ?? Math.Round(product.SellingPrice * (1m - (rule.DiscountPercent ?? 0m) / 100m), 2);

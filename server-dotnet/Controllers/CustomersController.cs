...
public sealed class CustomersController(BillingDbContext db) : ControllerBase
{
    ...
    {
        var customers = await db.Customers.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Name).ToListAsync(cancellationToken);
    ...
    public async Task<ActionResult<CustomerDto>> Create(CustomerRequest input, CancellationToken cancellationToken)
    {
        ...
        var companyExists = await db.Companies.AnyAsync(x => x.Id == input.CompanyId && x.IsActive, cancellationToken);
        if (!companyExists) return BadRequest("An active company is required.");

        var code = input.Code.Trim();
        if (await db.Customers.AnyAsync(x => x.CompanyId == input.CompanyId && x.Code == code, cancellationToken))
            return Conflict("A customer with this code already exists for the company.");

        var customer = FromRequest(input, code);
    ...
    public async Task<ActionResult<CustomerDto>> Update(Guid id, CustomerRequest input, CancellationToken cancellationToken)
    {
        ...
        var code = input.Code.Trim();
        var duplicate = await db.Customers.AnyAsync(x => x.Id != id && x.CompanyId == input.CompanyId && x.Code == code, cancellationToken);
        if (duplicate) return Conflict("A customer with this code already exists for the company.");
        ...
        customer.Gstin = input.Gstin?.Trim().ToUpperInvariant();
        customer.Address = input.Address?.Trim();
        customer.BillingAddress = input.BillingAddress?.Trim();
        customer.ShippingAddress = input.ShippingAddress?.Trim();
        customer.City = input.City?.Trim();
        customer.State = input.State?.Trim();
        customer.StateCode = input.StateCode?.Trim();
        customer.CustomerType = input.CustomerType.Trim().ToUpperInvariant();
    ...
    private static string? Validate(CustomerRequest input)
    {
        if (input.CompanyId == Guid.Empty) return "CompanyId is required.";
        if (string.IsNullOrWhiteSpace(input.Code)) return "Customer code is required.";
        if (string.IsNullOrWhiteSpace(input.Name)) return "Customer name is required.";
    ...
    private static bool IsValidGstin(string? gstin)
        => !string.IsNullOrWhiteSpace(gstin) && System.Text.RegularExpressions.Regex.IsMatch(gstin.Trim().ToUpperInvariant(), "^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$");

    private static Customer FromRequest(CustomerRequest input, string code) => new()
    {
        ...
        Gstin = input.Gstin?.Trim().ToUpperInvariant(),
        ...
        CustomerType = input.CustomerType.Trim().ToUpperInvariant(),

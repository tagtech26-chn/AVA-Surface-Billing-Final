...
{
    // Backward-compatible endpoint for existing POS/development callers.
    [HttpGet]
    public async Task<ActionResult<IEnumerable<CustomerDto>>> Get(CancellationToken cancellationToken)
        => Ok(await db.Customers.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Name)
    ...
    // Scalable endpoint for management screens.
    [HttpGet("page")]
    public async Task<ActionResult<CustomerPageDto>> GetPage([FromQuery] int page = 1, [FromQuery] int pageSize = 50, [FromQuery] string? search = null, [FromQuery] Guid? companyId = null, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<ActionResult<CustomerDto>> Create(CustomerRequest input, CancellationToken cancellationToken)
    {
        ...
        if (await db.Customers.AnyAsync(x => x.CompanyId == input.CompanyId && x.Code == code, cancellationToken)) return Conflict("A customer with this code already exists for the company.");
    ...
    public async Task<ActionResult<CustomerDto>> Update(Guid id, CustomerRequest input, CancellationToken cancellationToken)
    {
        ...
        var code = input.Code.Trim();
        if (await db.Customers.AnyAsync(x => x.Id != id && x.CompanyId == input.CompanyId && x.Code == code, cancellationToken)) return Conflict("A customer with this code already exists for the company.");
    ...
    {
        var customer = await db.Customers.FirstOrDefaultAsync(x => x.Id == id, cancellationToken); if (customer is null) return NotFound();
        customer.IsActive = false; await db.SaveChangesAsync(cancellationToken); return NoContent();
    }
    ...
    private static string? Validate(CustomerRequest input)
    {
        ...
        if (type == "B2B" && (string.IsNullOrWhiteSpace(input.StateCode) || !IsValidGstin(input.Gstin))) return "A valid GSTIN and state code are required for B2B customers.";

...
public sealed class CustomersController(BillingDbContext db) : ControllerBase
{
    ...
    private static string? Validate(CustomerRequest input)
    {
        ...
        if (type == "B2B") { if (string.IsNullOrWhiteSpace(input.StateCode)) return "State code is required for B2B customers."; if (!IsValidGstin(input.Gstin)) return "A valid GSTIN is required for B2B customers."; }

...
public sealed class ProductsController(BillingDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Update(Guid id, ProductRequest input, CancellationToken cancellationToken)
    {
        ...
        product.Sku = input.Sku.Trim(); product.Name = input.Name.Trim(); product.HsnCode = input.HsnCode; product.Unit = input.Unit ?? "PCS"; product.CostPrice = input.CostPrice; product.SellingPrice = input.SellingPrice; product.GstRate = input.GstRate; product.StockQuantity = input.Stock; product.ReorderLevel = input.ReorderLevel; product.IsActive = input.IsActive;
        await db.SaveChangesAsync(cancellationToken);
        return NoContent();
    }
    ...
    private static Product FromRequest(ProductRequest input, Guid companyId) => new() { CompanyId = companyId, Sku = input.Sku.Trim(), Name = input.Name.Trim(), HsnCode = input.HsnCode, Unit = input.Unit ?? "PCS", CostPrice = input.CostPrice, SellingPrice = input.SellingPrice, GstRate = input.GstRate, StockQuantity = input.Stock, ReorderLevel = input.ReorderLevel, IsActive = input.IsActive };

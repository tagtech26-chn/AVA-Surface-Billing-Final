...
public sealed class InvoiceWorkflowController(BillingDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Invoice>>> ManagerPending(CancellationToken cancellationToken)
        => Ok(await db.Invoices.AsNoTracking()
            .Include(x => x.Customer).Include(x => x.Salesperson)
            .Include(x => x.Lines).ThenInclude(x => x.Product)
            .Where(x => x.WorkflowStatus == "MANAGER_APPROVAL_PENDING")
            .OrderBy(x => x.InvoiceDate).ToListAsync(cancellationToken));
    ...
    public async Task<ActionResult> ApproveManagerDiscount(Guid invoiceId, ManagerApprovalRequest request, CancellationToken cancellationToken)
    {
        ...
        var originalLineBases = invoice.Lines.Select(line => new
        {
            Line = line,
        ...
        for (var index = 0; index < originalLineBases.Count; index++)
        {
            ...
            var managerDiscount = index == originalLineBases.Count - 1
        ...
        {
            UserId = manager.Id, Action = "INVOICE_MANAGER_DISCOUNT_APPROVED", EntityName = nameof(Invoice), EntityId = invoice.Id,
            Details = $"Additional discount {invoice.BranchManagerDiscountPercent:0.##}% approved. Remarks: {invoice.BranchManagerRemarks}", CreatedAtUtc = DateTime.UtcNow
        });
    ...
    public async Task<ActionResult> RejectManagerDiscount(Guid invoiceId, ManagerRejectionRequest request, CancellationToken cancellationToken)
    {
        ...
        invoice.BranchManagerRemarks = request.Remarks.Trim();
        db.AuditLogs.Add(new AuditLog { UserId = manager.Id, Action = "INVOICE_MANAGER_DISCOUNT_REJECTED", EntityName = nameof(Invoice), EntityId = invoice.Id, Details = $"Additional discount request rejected. Remarks: {invoice.BranchManagerRemarks}", CreatedAtUtc = DateTime.UtcNow });
        await db.SaveChangesAsync(cancellationToken);
        return Ok(invoice);
    }
    ...
    public async Task<ActionResult<IEnumerable<Invoice>>> WarehouseReady(CancellationToken cancellationToken)
        => Ok(await db.Invoices.AsNoTracking().Include(x => x.Customer).Include(x => x.Salesperson).Include(x => x.Lines).ThenInclude(x => x.Product).Where(x => x.WorkflowStatus == "PAYMENT_CONFIRMED" || x.WorkflowStatus == "WAREHOUSE_READY" || x.WorkflowStatus == "LOADED").OrderBy(x => x.InvoiceDate).ToListAsync(cancellationToken));
    ...
    public async Task<ActionResult> ConfirmPayment(Guid invoiceId, PaymentConfirmationRequest request, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Method)) return BadRequest("Payment method is required.");

        var requestedMethod = request.Method.Trim().ToUpperInvariant();
        var allowedMethods = new[] { "CASH", "CARD", "UPI_QR", "BANK_TRANSFER" };
        if (!allowedMethods.Contains(requestedMethod, StringComparer.OrdinalIgnoreCase)) return BadRequest("Payment method must be CASH, CARD, UPI_QR or BANK_TRANSFER.");
        if (string.IsNullOrWhiteSpace(request.SpecificReference)) return BadRequest("Payment-specific receipt/reference is required for Accounts confirmation.");
        ...
        if (requestedMethod is "UPI_QR" or "BANK_TRANSFER" && string.IsNullOrWhiteSpace(request.Utr)) return BadRequest("UTR / transaction ID is required for this payment method.");

        var invoice = await db.Invoices.AsNoTracking().FirstOrDefaultAsync(x => x.Id == invoiceId, cancellationToken);
        if (invoice is null) return NotFound();
        ...
        if (request.Amount > invoice.GrandTotal + 0.01m) return BadRequest("Payment amount cannot exceed the invoice grand total.");

        var paymentTime = request.PaymentDateUtc == default ? DateTime.UtcNow : request.PaymentDateUtc;
        ...
        var cardLast4 = string.IsNullOrWhiteSpace(request.CardLast4) ? null : request.CardLast4.Trim();
        var utr = string.IsNullOrWhiteSpace(request.Utr) ? null : request.Utr.Trim();
        var remarks = string.IsNullOrWhiteSpace(request.Remarks) ? null : request.Remarks.Trim();
        var newStatus = request.Amount >= invoice.GrandTotal ? "PAID" : "PARTIAL";

        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);
        try
        {
            // Use a set-based conditional update rather than a tracked entity update. This avoids
            // false EF concurrency failures on installations where invoice rows are mirrored/managed
            // by database-side infrastructure. The workflow state is part of the concurrency guard.
            var updated = await db.Invoices
                .Where(x => x.Id == invoiceId && x.WorkflowStatus == "PAYMENT_PENDING")
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(x => x.PaymentConfirmedByUserId, accountsUser.Id)
                    .SetProperty(x => x.PaymentConfirmedByName, accountsUser.DisplayName)
                    .SetProperty(x => x.PaymentConfirmedAtUtc, DateTime.UtcNow)
                    .SetProperty(x => x.PaymentMethodConfirmed, requestedMethod)
                    .SetProperty(x => x.PaymentSpecificReference, reference)
                    .SetProperty(x => x.PaymentBankName, bankName)
                    .SetProperty(x => x.PaymentCardLast4, cardLast4)
                    .SetProperty(x => x.PaymentUtr, utr)
                    .SetProperty(x => x.PaymentRemarks, remarks)
                    .SetProperty(x => x.WorkflowStatus, "PAYMENT_CONFIRMED")
                    .SetProperty(x => x.Status, newStatus), cancellationToken);

            if (updated != 1)
            {
                await transaction.RollbackAsync(cancellationToken);
                return Conflict("Payment was not confirmed because the invoice changed state. Refresh and try again.");
            }

            db.Payments.Add(new Payment
            {
                Id = Guid.NewGuid(), InvoiceId = invoiceId, Amount = request.Amount,
                Method = requestedMethod, PaymentDateUtc = paymentTime, Reference = reference
            });
            db.AuditLogs.Add(new AuditLog
            {
                UserId = accountsUser.Id, Action = "INVOICE_PAYMENT_CONFIRMED", EntityName = nameof(Invoice), EntityId = invoiceId,
                Details = $"Accounts confirmed {requestedMethod} payment: amount {request.Amount:0.00}, reference {reference}.", CreatedAtUtc = DateTime.UtcNow
            });
            await db.SaveChangesAsync(cancellationToken);
        ...
        var result = await db.Invoices.AsNoTracking().Include(x => x.Payments).FirstAsync(x => x.Id == invoiceId, cancellationToken);
        return Ok(result);
    }
    ...
    public async Task<ActionResult> MarkLoaded(Guid invoiceId, WarehouseLoadRequest request, CancellationToken cancellationToken)
    {
        ...
        db.AuditLogs.Add(new AuditLog { UserId = warehouseUser.Id, Action = "INVOICE_LOADED", EntityName = nameof(Invoice), EntityId = invoice.Id, Details = $"Loaded by {request.LoadedBy}; verified by {request.VerifiedBy}; vehicle {request.VehicleNumber}.", CreatedAtUtc = DateTime.UtcNow });
    ...
    public async Task<ActionResult> MarkDelivered(Guid invoiceId, DeliveryConfirmationRequest request, CancellationToken cancellationToken)
    {
        ...
        invoice.DeliveredAtUtc = DateTime.UtcNow; invoice.DeliveredByName = string.IsNullOrWhiteSpace(request.DeliveredByName) ? warehouseUser.DisplayName : request.DeliveredByName.Trim(); invoice.DeliveryRemarks = string.IsNullOrWhiteSpace(request.Remarks) ? null : request.Remarks.Trim(); invoice.WorkflowStatus = "DELIVERED";
        db.AuditLogs.Add(new AuditLog { UserId = warehouseUser.Id, Action = "INVOICE_DELIVERED", EntityName = nameof(Invoice), EntityId = invoice.Id, Details = $"Delivered by {invoice.DeliveredByName}. Remarks: {request.Remarks}", CreatedAtUtc = DateTime.UtcNow });
        await db.SaveChangesAsync(cancellationToken); return Ok(invoice);
    }

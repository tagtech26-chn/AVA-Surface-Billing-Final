...
public sealed class InvoiceWorkflowController(BillingDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult> ApproveManagerDiscount(Guid invoiceId, ManagerApprovalRequest request, CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < bases.Count; i++)
        {
            ...
            if (invoice.IgstAmount > 0m) { item.Line.CgstAmount = 0m; item.Line.SgstAmount = 0m; item.Line.IgstAmount = tax; }
            else { item.Line.CgstAmount = Math.Round(tax / 2m, 2, MidpointRounding.AwayFromZero); item.Line.SgstAmount = Math.Round(tax - item.Line.CgstAmount, 2, MidpointRounding.AwayFromZero); item.Line.IgstAmount = 0m; }
    ...
    public async Task<ActionResult> ConfirmPayment(Guid invoiceId, PaymentConfirmationRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!new[] { "CASH", "CARD", "UPI_QR", "BANK_TRANSFER" }.Contains(requestedMethod)) return BadRequest("Invalid payment method.");
        ...
        try
        {
            var updated = await db.Invoices.Where(x => x.Id == invoiceId && x.WorkflowStatus == "PAYMENT_PENDING").ExecuteUpdateAsync(setters => setters
                ...
                .SetProperty(x => x.PaymentBankName, string.IsNullOrWhiteSpace(request.BankName) ? null : request.BankName.Trim())
                .SetProperty(x => x.PaymentCardLast4, string.IsNullOrWhiteSpace(request.CardLast4) ? null : request.CardLast4.Trim())
                .SetProperty(x => x.PaymentUtr, string.IsNullOrWhiteSpace(request.Utr) ? null : request.Utr.Trim())
                .SetProperty(x => x.PaymentRemarks, string.IsNullOrWhiteSpace(request.Remarks) ? null : request.Remarks.Trim())
    ...
    public async Task<ActionResult> MarkLoaded(Guid invoiceId, WarehouseLoadRequest request, CancellationToken cancellationToken)
    {
        ...
        invoice.WarehouseLoadedBy = request.LoadedBy.Trim(); invoice.WarehouseVerifiedBy = request.VerifiedBy.Trim(); invoice.WarehouseLoadedAtUtc = DateTime.UtcNow; invoice.WarehouseVehicleNumber = string.IsNullOrWhiteSpace(request.VehicleNumber) ? null : request.VehicleNumber.Trim(); invoice.WarehouseRemarks = string.IsNullOrWhiteSpace(request.Remarks) ? null : request.Remarks.Trim();
        invoice.WorkflowStatus = "COMPLETED";

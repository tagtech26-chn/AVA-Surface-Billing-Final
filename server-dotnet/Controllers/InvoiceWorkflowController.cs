...
public sealed class InvoiceWorkflowController(BillingDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult> ResolveWorkflowUser([FromQuery] string role, CancellationToken cancellationToken)
    {
        ...
        if (normalized is not "ACCOUNTANT" and not "WAREHOUSE" and not "BRANCH_MANAGER" and not "ADMIN" and not "CASHIER")
    ...
    public async Task<ActionResult<IEnumerable<Invoice>>> CashierDiscountCandidates(CancellationToken cancellationToken)
    ...
    public async Task<ActionResult> RequestManagerDiscount(Guid invoiceId, ManagerDiscountRequest request, CancellationToken cancellationToken)
    {
        ...
        if (invoice.WorkflowStatus != "PAYMENT_PENDING" && invoice.WorkflowStatus != "MANAGER_APPROVAL_REJECTED")
            return BadRequest($"Invoice is currently in workflow state '{invoice.WorkflowStatus}' and cannot be submitted for manager approval.");
        ...
        invoice.BranchManagerRemarks = string.IsNullOrWhiteSpace(request.Remarks)
            ? $"Additional discount requested by {request.RequestedByName.Trim()}."
            : $"Additional discount requested by {request.RequestedByName.Trim()}. {request.Remarks.Trim()}";
    ...
    public sealed record ManagerDiscountRequest(string RequestedByName, string? Remarks = null);

...
const ASPNET_PROXY_PREFIXES = ["/api/auth", "/api/invoice-workflow", "/api/invoices", "/api/customers", "/api/companies", "/api/products", "/api/promotions", "/api/stock", "/api/expenses", "/api/salespersons", "/api/gst-verification", "/api/manager", "/api/accounts", "/api/audit-logs", "/api/drafts", "/api/enterprise"];

app.post("/api/invoices", async (req, res, next) => {
  try {
    const lines = Array.isArray(req.body?.lines) ? req.body.lines : [];
    const productIds: string[] = Array.from(new Set<string>(lines.map((line: any) => String(line?.productId || "").trim()).filter((id: string) => Boolean(id))));
    if (productIds.length === 0) return next();
    const authorization = req.get("authorization");
    for (const productId of productIds) {
      const headers: Record<string, string> = {};
      if (authorization) headers.authorization = authorization;
      const productResponse = await fetch(`${DOTNET_API_URL}/api/products/${encodeURIComponent(productId)}`, { method: "GET", headers });
      if (productResponse.status === 404) return res.status(400).json({ error: `Product ${productId} is not available in the inventory master.` });
      ...
      if (product.isActive === false) return res.status(400).json({ error: `Product "${product.name || productId}" is deactivated and cannot be billed.` });
    }
    return next();
  } catch (error) { console.error("Inactive-product billing guard failed:", error); return res.status(502).json({ error: "Unable to validate inventory status before billing." }); }
});
...
app.use(ASPNET_PROXY_PREFIXES, async (req, res, next) => {
  try {
    ...
    const headers: Record<string, string> = {};
    const authorization = req.get("authorization"); const contentType = req.get("content-type"); const accept = req.get("accept");
    if (authorization) headers.authorization = authorization; if (contentType) headers["content-type"] = contentType; if (accept) headers.accept = accept;
    const upstream = await fetch(targetUrl, { method: req.method, headers, body: !["GET", "HEAD"].includes(req.method) ? JSON.stringify(req.body) : undefined });
    const responseText = await upstream.text(); res.status(upstream.status); const upstreamContentType = upstream.headers.get("content-type"); if (upstreamContentType) res.set("content-type", upstreamContentType); return res.send(responseText);
  } catch (error) { console.error(`ASP.NET API proxy failed for ${req.method} ${req.originalUrl}:`, error); return next(error); }
});

let invoiceNumberQueue = Promise.resolve();
app.post("/api/invoice-number", async (_req, res, next) => {
  const task = invoiceNumberQueue.then(async () => { const now = new Date(); const fiscalStartYear = now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1; const fy = `${String(fiscalStartYear).slice(-2)}${String(fiscalStartYear + 1).slice(-2)}`; const current = await getEntity("invoiceSequence", { fiscalYear: fy, nextNumber: 1 }); const nextNumber = current?.fiscalYear === fy && Number(current?.nextNumber) > 0 ? Number(current.nextNumber) : 1; await saveEntity("invoiceSequence", { fiscalYear: fy, nextNumber: nextNumber + 1, updatedAt: new Date().toISOString() }); return `AVA-${String(nextNumber).padStart(4, "0")}-${fy}`; });
  invoiceNumberQueue = task.then(() => undefined, () => undefined); try { return res.json({ invoiceNumber: await task }); } catch (error) { console.error("Invoice number allocation failed:", error); return next(error); }
});

app.get("/api/health", async (_req, res) => { try { await getEntity("__health__", { initialized: false }); res.json({ status: "ok", persistence: "server", database: "sqlite", timestamp: new Date().toISOString() }); } catch (error) { console.error("Health check failed:", error); res.status(503).json({ status: "error", persistence: "server" }); } });

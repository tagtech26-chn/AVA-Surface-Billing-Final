...
export interface UserProfile { id: string; username?: string; name: string; email: string; role: UserRole; pin: string; avatar: string; phone?: string; }
export interface Product { id: string; sku: string; barcode: string; name: string; category: string; costPrice: number; sellingPrice: number; stock: number; reorderLevel: number; taxRate: number; unit: string; hsnCode?: string; description?: string; imageUrl?: string; updatedAt: string; isActive?: boolean; tileDimensions?: string; pcsPerBox?: number; sqftPerBox?: number; tileFinish?: string; tileType?: string; batchNo?: string; pricePerSqFt?: number; weightPerBoxKg?: number; sellingPricePerPcs?: number; sellingPricePerBox?: number; sellingPricePerSqFt?: number; sellingPricePerSqMt?: number; sellingPricePerSet?: number; }
export interface StockAdjustment { id: string; productId: string; productName: string; quantityChange: number; type: 'RESTOCK' | 'DAMAGE' | 'LOSS' | 'AUDIT_CORRECTION'; reason: string; performedBy: string; date: string; }
export interface Customer { id: string; name: string; phone: string; email?: string; customerType?: 'NORMAL' | 'LEDGER'; gstNumber?: string; gstLegalName?: string; gstTradeName?: string; gstStatus?: 'ACTIVE' | 'INACTIVE' | 'SUSPENDED'; gstState?: string; gstAddress?: string; taxNumber?: string; address?: string; billingAddress?: string; shippingAddress?: string; city?: string; state?: string; stateCode?: string; loyaltyPoints: number; totalSpent: number; outstandingBalance: number; }
...
export interface ManagerDiscountApproval { id: string; invoiceId: string; invoiceNumber: string; customerName: string; requestedByUserId: string; requestedByName: string; requestedAt: string; status: ManagerApprovalStatus; approvedDiscountAmount?: number; approvedByUserId?: string; approvedByName?: string; approvedAt?: string; rejectionReason?: string; items: CartItem[]; subtotal: number; existingDiscountAmount: number; taxTotal: number; currentGrandTotal: number; }
...
export interface AuditLog { id: string; timestamp: string; category: AuditCategory; severity: AuditSeverity; action: string; performedBy: string; performedByRole: UserRole; targetId?: string; targetName?: string; details: string; previousValue?: string; newValue?: string; ipAddress?: string; }
export interface TallyLedgerMapping { salesLedger: string; cashLedger: string; bankLedger: string; cgstLedger: string; sgstLedger: string; igstLedger: string; debtorsGroup: string; companyName: string; }
export interface AIInsightResponse { insight: string; recommendations: string[]; promoIdea?: { code: string; discount: string; description: string; }; }

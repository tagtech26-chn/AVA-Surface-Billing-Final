...
function mapLine(line: NonNullable<ServerInvoice['lines']>[number]): CartItem {
  ...
  const product: Product = {
    id: line.productId,
    sku: serverProduct?.sku || line.productId,
    barcode: serverProduct?.sku || line.productId,
    name: serverProduct?.name || 'Item',
    category: 'General',
    costPrice: 0,
    sellingPrice: number(line.unitPrice),
    stock: number(serverProduct?.stock),
    reorderLevel: number(serverProduct?.reorderLevel),
    taxRate: number(serverProduct?.gstRate),
    unit: serverProduct?.unit || 'unit',
  ...
  return {
    ...
    inputQuantity: number(line.quantity),
...
function mapInvoice(source: ServerInvoice): Invoice {
  const payments = (source.payments || []).map((payment) => ({
    ...
    method: (payment.method || source.paymentMethodConfirmed || source.paymentMethodRequested || 'CASH') as PaymentMethod,
  ...
  return {
    ...
    cashierName: 'Billing',
    cashierRole: 'BILLING_USER',
    salespersonName: source.salespersonName || source.salesperson?.name,
    salespersonMobile: source.salespersonMobile || source.salesperson?.mobile,
    items: (source.lines || []).map(mapLine),
    subtotal: number(source.subTotal ?? source.subtotal),
    itemDiscountsTotal: number(source.discountAmount),
    promoDiscountAmount: number(source.promoDiscountAmount),
    branchManagerDiscountPercent: number(source.branchManagerDiscountPercent),
    branchManagerDiscountAmount: number(source.branchManagerDiscountAmount),
    branchManagerRemarks: source.branchManagerRemarks,
    manualDiscountAmount: number(source.branchManagerDiscountAmount),
    taxTotal: number(source.taxableAmount ? number(source.cgstAmount) + number(source.sgstAmount) + number(source.igstAmount) : source.taxAmount),
    cgstAmount: number(source.cgstAmount),
    sgstAmount: number(source.sgstAmount),
    igstAmount: number(source.igstAmount),
    roundOffAmount: number(source.roundOffAmount),
    grandTotal: number(source.grandTotal),
    amountPaid,
    changeGiven: 0,
    status,
    paymentMethod: (source.paymentMethodConfirmed || source.paymentMethodRequested || 'CASH') as PaymentMethod,

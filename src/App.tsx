...
export default function App() {
  ...
  const handleUpdateDeliveryStatus = (invoiceId: string, deliveryStatus: DeliveryStatus, dispatchDetails?: { driverName?: string; driverPhone?: string; vehicleNumber?: string; transporterName?: string; trackingNumber?: string; deliveryNotes?: string; }) => {
    setInvoices((prev) => prev.map((inv) => inv.id === invoiceId ? { ...inv, deliveryStatus, dispatchDate: new Date().toISOString(), ...(dispatchDetails || {}) } : inv));
  };
  ...
  const handleAddNewCustomer = (newCustData: Omit<Customer, 'id' | 'loyaltyPoints' | 'totalSpent' | 'outstandingBalance'>): Customer => {
    const newCust: Customer = { ...newCustData, id: `cust-${Date.now()}`, loyaltyPoints: 0, totalSpent: 0, outstandingBalance: 0 };
  ...
  const handleSaveProduct = (product: Product) => {
    ...
    if (existing) {
      const priceChanged = existing.sellingPrice !== product.sellingPrice || existing.costPrice !== product.costPrice;
      logAudit('PRODUCT', priceChanged ? 'HIGH' : 'MEDIUM', priceChanged ? 'Product Price Modified' : 'Product Details Updated', priceChanged ? `Updated selling price of ${product.name} from ${storeDetails.currencySymbol}${existing.sellingPrice} to ${storeDetails.currencySymbol}${product.sellingPrice}.` : `Updated product attributes for ${product.name}.`, product.name, product.id, `${storeDetails.currencySymbol}${existing.sellingPrice}`, `${storeDetails.currencySymbol}${product.sellingPrice}`);
    } else {
      logAudit('PRODUCT', 'MEDIUM', 'New Product Created', `Created product entry ${product.name} (SKU: ${product.sku}) with selling price ${storeDetails.currencySymbol}${product.sellingPrice}.`, product.name, product.id, 'Non-Existent', `${storeDetails.currencySymbol}${product.sellingPrice}`);
    }
    setProducts((prev) => prev.some((p) => p.id === product.id) ? prev.map((p) => p.id === product.id ? product : p) : [product, ...prev]);
  };

  const handleStockAdjustment = (adjustment: StockAdjustment) => {
    setStockLogs((prev) => [adjustment, ...prev]);
    logAudit('STOCK', 'MEDIUM', 'Physical Inventory Stock Adjustment', `Adjusted stock for ${adjustment.productName}. Change: ${adjustment.quantityChange > 0 ? '+' : ''}${adjustment.quantityChange} units (${adjustment.reason}).`, adjustment.productName, adjustment.productId, 'Adjustment Type', `${adjustment.type} (${adjustment.quantityChange > 0 ? '+' : ''}${adjustment.quantityChange})`);
  };

  const handleSavePromo = (promo: PromoRule) => {
    const existing = promos.find((p) => p.id === promo.id);
    logAudit('PROMO', 'MEDIUM', existing ? 'Promo Offer Updated' : 'New Promo Offer Created', `Saved promo rule ${promo.code} (${promo.description}).`, promo.code, promo.id);
    setPromos((prev) => prev.some((p) => p.id === promo.id) ? prev.map((p) => p.id === promo.id ? promo : p) : [promo, ...prev]);
  };
  ...
  const handleTogglePromoActive = (promoId: string) => {
    ...
    if (target) {
      ...
      logAudit('PROMO', 'MEDIUM', nextState ? 'Promo Offer Activated' : 'Promo Offer Deactivated', `${nextState ? 'Enabled' : 'Disabled'} promo code ${target.code}.`, target.code, target.id, target.isActive ? 'Active' : 'Inactive', nextState ? 'Active' : 'Inactive');
    }
    setPromos((prev) => prev.map((p) => p.id === promoId ? { ...p, isActive: !p.isActive } : p));
  };

  const handleRecordInvoicePayment = (invoiceId: string, paymentAmount: number, method: PaymentMethod, notes?: string) => {
    const targetInv = invoices.find((i) => i.id === invoiceId);
    logAudit('INVOICE', 'MEDIUM', 'Invoice AR Payment Received', `Recorded payment of ${storeDetails.currencySymbol}${paymentAmount.toFixed(2)} via ${method} for invoice ${targetInv?.invoiceNumber || invoiceId}.`, targetInv?.invoiceNumber || invoiceId, invoiceId);
    setInvoices((prev) => prev.map((inv) => {
      if (inv.id !== invoiceId) return inv;
      const newAmountPaid = inv.amountPaid + paymentAmount;
      const newStatus = newAmountPaid >= inv.grandTotal ? 'PAID' : 'PARTIAL';
      const newPaymentRecord = { id: `pay-${Date.now()}`, amount: paymentAmount, method, date: new Date().toISOString(), notes: notes || 'AR Payment Received' };
      return { ...inv, amountPaid: newAmountPaid, status: newStatus, paymentsHistory: [...inv.paymentsHistory, newPaymentRecord] };
    }));
  };

  const handleProcessRefund = (invoiceId: string, restockItems: boolean) => {
    const targetInv = invoices.find((i) => i.id === invoiceId);
    logAudit('INVOICE', 'HIGH', 'Invoice Refund Processed', `Processed cancellation & refund for invoice ${targetInv?.invoiceNumber || invoiceId}. Stock auto-restocked: ${restockItems ? 'YES' : 'NO'}.`, targetInv?.invoiceNumber || invoiceId, invoiceId, `Status: ${targetInv?.status || 'PAID'}`, 'Status: REFUNDED');
    setInvoices((prev) => prev.map((inv) => {
      if (inv.id !== invoiceId) return inv;
      if (restockItems) {
        setProducts((prodList) => prodList.map((prod) => {
  ...
  const handleCreateUser = (newUser: Omit<UserProfile, 'id'>) => {
    const created: UserProfile = { ...newUser, id: `u-${Date.now()}` };
    setUsers((prev) => [...prev, created]);
    logAudit('USER', 'CRITICAL', 'Staff Account & Role Created', `Created staff account for ${created.name} (${created.email}) assigned to role ${created.role}.`, created.name, created.id, 'Unassigned', `Role: ${created.role}`);
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      {showLowStockToast && lowStockCount > 0 && (
        <div className="bg-gradient-to-r from-amber-950 via-slate-900 to-rose-950 border-b border-amber-500/30 px-4 py-2.5 text-xs text-amber-200 flex items-center justify-between shadow-xl sticky top-16 z-20 transition-all">
          <div className="flex items-center space-x-3 overflow-hidden"><span className="flex h-2.5 w-2.5 rounded-full bg-rose-500 animate-ping shrink-0" /><div className="flex items-center space-x-2 truncate"><span className="font-black tracking-wide text-rose-300 bg-rose-500/20 px-2 py-0.5 rounded border border-rose-500/30 shrink-0">⚠️ REORDER ALERT ({lowStockCount})</span><span className="hidden sm:inline font-medium text-slate-200 truncate">Logged in as <strong className="text-white font-bold">{activeUser.name}</strong>. {lowStockCount} product{lowStockCount > 1 ? 's' : ''} at or below reorder threshold:</span><span className="font-semibold text-amber-300 truncate">{lowStockProducts.slice(0, 3).map((p) => `${p.name} (${p.stock} left)`).join(', ')}{lowStockCount > 3 ? ` +${lowStockCount - 3} more` : ''}</span></div></div>
          <div className="flex items-center space-x-2 shrink-0 ml-3"><button onClick={() => setActiveTab('inventory')} className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold rounded-lg transition shadow text-[11px]">Review Inventory</button><button onClick={() => setShowLowStockToast(false)} className="p-1 text-slate-400 hover:text-white transition" title="Dismiss Alert"><X className="w-4 h-4" /></button></div>
        </div>
      )}
      <div className="flex-1 flex overflow-hidden">
        <Navigation activeTab={activeTab} setActiveTab={setActiveTab} userRole={activeUser.role} lowStockCount={lowStockCount} activePromoCount={activePromoCount} unpaidInvoiceCount={unpaidInvoiceCount} pendingDispatchCount={pendingDispatchCount} />

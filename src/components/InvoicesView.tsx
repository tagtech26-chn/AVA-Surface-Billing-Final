...
import { AlertCircle, Calendar, CheckCircle2, Clock, Eye, Printer, Receipt, RefreshCw, ShieldCheck, Timer } from 'lucide-react';
  ...
  return <div className="space-y-5">
    ...
    {visibleInvoices.length === 0 ? <div className="p-10 text-center bg-slate-900 border border-slate-800 rounded-3xl text-slate-500">No {documentLabel.toLowerCase()}s found.</div> : <div className="grid gap-4 lg:grid-cols-[420px_1fr]">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">{!selected ? <div className="min-h-60 flex items-center justify-center text-slate-500">Select a {documentLabel.toLowerCase()} to view details.</div> : <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs"><div className="p-3 rounded-xl bg-slate-950 border border-slate-800"><div className="text-slate-500">Workflow</div><div className="font-black text-white mt-1">{selected.workflowStatus || (isFinalInvoice(selected) ? 'PAYMENT_CONFIRMED' : 'PAYMENT_PENDING')}</div></div><div className="p-3 rounded-xl bg-slate-950 border border-slate-800"><div className="text-slate-500">Payment Method</div><div className="font-black text-white mt-1">{selected.paymentMethodConfirmed || selected.paymentMethod}</div></div><div className="p-3 rounded-xl bg-slate-950 border border-slate-800"><div className="p-3 rounded-xl bg-slate-950 border border-slate-800"><div className="text-slate-500">Payment Reference</div><div className="font-black text-white mt-1 break-all">{paymentReference || '—'}</div>{paymentDate && <div className="text-[10px] text-slate-500 mt-1">{formatDateTime(paymentDate)}</div>}</div></div>

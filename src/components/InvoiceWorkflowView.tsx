...
export const InvoiceWorkflowView: React.FC<Props> = ({ activeUser, currencySymbol, onPaymentConfirmed }) => {
  ...
  useEffect(() => { if (!isWarehouse) void load(); }, [activeUser.role, tab]);
  ...
  const renderTabs = () => <div className="flex items-center gap-2 mb-4"><button onClick={() => setTab('unapproved')} className={`px-4 py-2 rounded-xl text-xs font-black ${tab === 'unapproved' ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-300'}`}>Unapproved Invoices</button><button onClick={() => setTab('approved')} className={`px-4 py-2 rounded-xl text-xs font-black ${tab === 'approved' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-300'}`}>Approved Invoices</button></div>;

  const renderManager = () => <>
    {renderTabs()}
    {invoices.map(invoice => {
      const editable = tab === 'unapproved' && (invoice.workflowStatus === 'MANAGER_APPROVAL_PENDING' || invoice.workflowStatus === 'MANAGER_APPROVAL_REJECTED');
      const isSelected = selectedId === invoice.id;
      const paid = invoice.workflowStatus === 'PAYMENT_CONFIRMED' || invoice.workflowStatus === 'COMPLETED' || invoice.status === 'PAID';
  ...
  const renderAccounts = () => <>
    ...
    {invoices.map(invoice => {
      const isSelected = selectedId === invoice.id;
      const net = Math.max(0, Number(invoice.grandTotal || 0) - Number(invoice.creditNoteAmount || 0));
      const canCollect = tab === 'approved' && invoice.workflowStatus === 'PAYMENT_PENDING';
      const paid = invoice.workflowStatus === 'PAYMENT_CONFIRMED' || invoice.workflowStatus === 'COMPLETED' || invoice.status === 'PAID';
      return <div key={invoice.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-3"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><div className="flex items-center gap-2"><div className="font-mono font-black text-white">{invoice.invoiceNumber}</div>{paid && <Lock className="w-3.5 h-3.5 text-emerald-400" />}</div><div className="text-xs text-slate-300 mt-1">{invoice.customer?.name || 'Customer'} · Salesperson: {invoice.salesperson?.name || invoice.salespersonName || '—'}</div><div className="text-[11px] text-slate-400 mt-1">Status: <b className="text-slate-200">{invoice.workflowStatus || invoice.status}</b></div><div className="text-[11px] text-slate-400 mt-1">Invoice value: <b className="text-white">{formatCurrency(invoice.grandTotal, currencySymbol)}</b> · Credit note: <b className="text-amber-300">{formatCurrency(invoice.creditNoteAmount || 0, currencySymbol)}</b></div></div><div className="flex items-center gap-3"><div className="text-right"><div className="text-[10px] text-slate-500">{paid ? 'Amount collected' : 'Amount to collect'}</div><div className={`font-black ${paid ? 'text-emerald-400' : 'text-emerald-400'}`}>{formatCurrency(net, currencySymbol)}</div></div>{canCollect && <button onClick={() => { setSelectedId(isSelected ? null : invoice.id); setMethod((invoice.paymentMethodRequested || 'CASH').toUpperCase()); setAmount(net.toFixed(2)); setReference(''); setBankName(''); setCardLast4(''); setUtr(''); setRemarks(''); }} className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-bold text-white">{isSelected ? 'Close' : 'Collect Payment'}</button>}</div></div>{isSelected && canCollect && <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-3"><select value={method} onChange={e => setMethod(e.target.value)} className={inputClass}><option value="CASH">Cash</option><option value="CARD">Card</option><option value="UPI_QR">UPI / QR</option><option value="BANK_TRANSFER">Bank Transfer</option></select><input value={amount} onChange={e => setAmount(e.target.value)} type="number" step="0.01" className={inputClass} placeholder="Amount received"/><input value={reference} onChange={e => setReference(e.target.value)} className={inputClass} placeholder="Receipt / payment reference *" />{method === 'CARD' && <input value={cardLast4} onChange={e => setCardLast4(e.target.value.replace(/\D/g, '').slice(0,4))} className={inputClass} placeholder="Card last 4 digits *" />}{(method === 'UPI_QR' || method === 'BANK_TRANSFER') && <input value={utr} onChange={e => setUtr(e.target.value)} className={inputClass} placeholder="UTR / transaction ID *" />}{method === 'BANK_TRANSFER' && <input value={bankName} onChange={e => setBankName(e.target.value)} className={inputClass} placeholder="Bank name" />}<textarea value={remarks} onChange={e => setRemarks(e.target.value)} className={`${inputClass} md:col-span-2`} placeholder="Accounts remarks" /><button onClick={() => void confirmPayment()} className="md:col-span-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-black text-white flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4" /> Confirm Payment &amp; Keep Invoice in Approved</button></div>}</div>;
    })}
  </>;
  ...
  return <div className="space-y-5"><div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex items-center gap-3">{isManager ? <ClipboardList className="w-6 h-6 text-violet-400"/> : isAccounts ? <CreditCard className="w-6 h-6 text-emerald-400"/> : <PackageCheck className="w-6 h-6 text-amber-400"/>}<div><h2 className="text-xl font-black text-white">{isManager ? 'Manager Invoice Review' : isAccounts ? 'Accounts Invoice Register' : 'Warehouse Loading & Completion'}</h2><p className="text-xs text-slate-400">Logged in as {activeUser.name} · {activeUser.role}</p></div></div>{message && <div className="p-3 rounded-xl bg-indigo-950/50 border border-indigo-500/30 text-xs text-indigo-200">{message}</div>}{loading ? <div className="p-8 text-center text-slate-400">Loading invoices...</div> : invoices.length === 0 ? <div className="p-10 text-center bg-slate-900 border border-slate-800 rounded-3xl text-slate-500">No invoices in this section.</div> : isManager ? renderManager() : isAccounts ? renderAccounts() : renderWarehouse()}</div>;
};

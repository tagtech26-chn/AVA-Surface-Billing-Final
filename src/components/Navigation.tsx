...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const [isCollapsed, setIsCollapsed] = useState<boolean>(
    () => localStorage.getItem('bizflow_nav_collapsed') === 'true'
  );
  ...
  const navItems = [
    ...
    ...(isBillingRole
      ? [
          {
            id: 'discount-request',
            label: 'Additional Discount Request',
            shortLabel: 'Discount',
            icon: Send,
            action: 'REQUEST_MANAGER_DISCOUNT' as const,
            badge: null
          }
        ]
      : [
          {
            id: 'accounts',
            label: isManagerRole
              ? 'Additional Discount Approval'
              : 'Accounts Payment Confirmation',
            shortLabel: isManagerRole ? 'Approvals' : 'Accounts',
            icon: isManagerRole ? Percent : CreditCard,
            action: isManagerRole
              ? ('APPROVE_BRANCH_MANAGER_DISCOUNT' as const)
              : ('CONFIRM_PAYMENTS' as const),
            badge:
              unpaidInvoiceCount > 0 && !isManagerRole
                ? `${unpaidInvoiceCount}`
                : null,
            badgeColor: isManagerRole
  ...
  const visibleItems = navItems.filter((item) =>
    canPerformAction(userRole, item.action)
  );
  ...
  return (
    <>
      <aside
        className={`hidden md:flex flex-col bg-slate-900 border-r border-slate-800 p-3 shrink-0 space-y-4 min-h-[calc(100vh-4rem)] transition-all duration-300 ${
      ...
      >
        <div
          ...
          } pt-1 pb-2 border-b border-slate-800/80`}
        >
          {!isCollapsed && (
            ...
            {visibleItems.map((item) => {
              ...
              return (
                <button
                  ...
                  } rounded-xl font-medium text-xs transition group ${
                    isActive
                  ...
                  >
                    <div className="relative shrink-0">
                      ...
                      {isCollapsed && item.badge && (
                        <span className="absolute -top-1.5 -right-2 px-1 min-w-[14px] text-[9px] font-extrabold rounded-full bg-rose-500 text-white flex items-center justify-center">
                          {item.badge}
                        </span>
                      )}
                    </div>
        ...
        {!isCollapsed && (
          <div className="mt-auto p-3.5 rounded-2xl bg-slate-800/80 border border-slate-700/80 text-xs text-slate-300">
            <div className="flex items-center justify-between mb-1">
              <span className="font-semibold text-white">Access Level</span>

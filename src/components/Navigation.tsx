...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const navItems = [
    ...
    ...(isCashier
      ? [{ id: 'discount-request', label: 'Additional Discount Request', shortLabel: 'Discount', icon: Send, action: 'REQUEST_MANAGER_DISCOUNT' as const, badge: null }]
      : [{ id: 'accounts', label: isManagerRole ? 'Additional Discount Approval' : 'Accounts Payment Confirmation', shortLabel: isManagerRole ? 'Approvals' : 'Accounts', icon: isManagerRole ? Percent : CreditCard, action: isManagerRole ? 'APPROVE_BRANCH_MANAGER_DISCOUNT' as const : 'CONFIRM_PAYMENTS' as const, badge: unpaidInvoiceCount > 0 && !isManagerRole ? `${unpaidInvoiceCount}` : null, badgeColor: isManagerRole ? 'bg-violet-500/20 text-violet-300 border-violet-500/40' : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' }]),
  ...
  const roleDescription = {
    ...
    MANAGER: 'POS sales, inventory, promotions, financial review and additional discount approval.',
  ...
  return (
    <>
      <aside className={`hidden md:flex flex-col bg-slate-900 border-r border-slate-800 p-3 shrink-0 space-y-4 min-h-[calc(100vh-4rem)] transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <div className="flex-1 px-1"><nav className="space-y-1.5">{visibleItems.map((item) => { const Icon = item.icon; const isActive = activeTab === item.id; return <button key={item.label} onClick={() => setActiveTab(item.id)} title={item.label} className={`w-full flex items-center ${isCollapsed ? 'justify-center py-3 px-2' : 'justify-between px-3 py-2.5'} rounded-xl font-medium text-xs transition group ${isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}><div className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} min-w-0`}><div className="relative shrink-0"><Icon className={`w-4 h-4 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-400'}`} />{isCollapsed && item.badge && <span className="absolute -top-1.5 -right-2 px-1 min-w-[14px] text-[9px] font-extrabold rounded-full bg-rose-500 text-white flex items-center justify-center">{item.badge}</span>}</div>{!isCollapsed && <span className="truncate font-medium">{item.label}</span>}</div>{!isCollapsed && item.badge && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${item.badgeColor || 'bg-slate-800 text-slate-300 border-slate-700'}`}>{item.badge}</span>}</button>; })}</nav></div>
        {!isCollapsed && <div className="mt-auto p-3.5 rounded-2xl bg-slate-800/80 border border-slate-700/80 text-xs text-slate-300"><div className="flex items-center justify-between mb-1"><span className="font-semibold text-white">Access Level</span><span className="text-[10px] uppercase font-bold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">{userRole}</span></div><p className="text-[11px] text-slate-400 leading-relaxed">{roleDescription}</p></div>}
      </aside>
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 px-1 py-1.5 flex justify-around items-center shadow-2xl overflow-x-auto">{visibleItems.map((item) => { const Icon = item.icon; const isActive = activeTab === item.id; return <button key={item.label} onClick={() => setActiveTab(item.id)} title={item.label} className={`flex flex-col items-center justify-center py-1 px-2 rounded-xl transition relative shrink-0 ${isActive ? 'text-indigo-400 font-bold' : 'text-slate-400 hover:text-slate-200'}`}><div className="relative"><Icon className="w-4 h-4" />{item.badge && <span className="absolute -top-1 -right-1.5 w-2 h-2 rounded-full bg-rose-500" /></div><span className="text-[9px] mt-0.5 tracking-tight">{item.shortLabel}</span></button>; })}</nav>
    </>
  );
};
